...
static mongo_connection* removeFirst( mongo_connection_pool *pool ) {
    ...
    pool->lock = CreateMutex( NULL, TRUE, CONNECTION_POOL_MUTEX_NAME );
    WaitForSingleObject( pool->lock, INFINITE );

    pool->head = pool->head->next;

    ReleaseMutex( pool->lock );
...
static void addToDictionary( mongo_connection_dictionary *dict, mongo_connection_pool *pool, mongo_connection_pool *lastPoolInDict ) {
    dict->lock = CreateMutex( NULL, TRUE, DICTIONARY_MUTEX_NAME );
    WaitForSingleObject( dict->lock, INFINITE );
