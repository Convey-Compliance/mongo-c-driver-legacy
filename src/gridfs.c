...
static bson *chunk_new(bson_oid_t id, int chunkNumber, const char *data, int len) {
...
int gridfs_init(mongo *client, const char *dbname, const char *prefix, gridfs *gfs) {
  ...
  gfs->files_ns = (const char*)bson_malloc(strlen(prefix) + strlen(dbname) + strlen(".files") + 2);
  ...
  gfs->chunks_ns = (const char*)bson_malloc(strlen(prefix) + strlen(dbname) + strlen(".chunks") + 2);
...
char* upperFileName(const char* filename){
  char* upperName = (char*) bson_malloc( strlen( filename ) + 1 );
  strcpy(upperName, filename);
  _strupr(upperName);
  return upperName;
}

static int gridfs_insert_file(gridfs *gfs, const char *name, const bson_oid_t id, gridfs_offset length, const char *contenttype) {
  bson command;
  bson ret;
  bson res;
  bson_iterator it;
  ...
  /* If you don't care about calculating MD5 hash for a particular file, simply include the string "no_md5" on your contentype param */
  if( !strstr( contenttype, "no_md5" ) ) {  
    /* Check run md5 */
    bson_init(&command);
...
void gridfile_init_length(gridfile *gfile) {
  bson_iterator it;
...
void gridfile_prepare_chunk_key_bson(bson *q, bson_oid_t *id, int chunk_num) {
  bson_init(q);
...
void gridfile_flush_pendingchunk(gridfile *gfile) {
  bson *oChunk;
  bson q;
...
int gridfile_load_pending_data_with_pos_chunk(gridfile *gfile) {
  int chunk_len;
  ...
  gfile->pending_len = chunk_len;
  gfile->chunk_num = (int)(gfile->pos / DEFAULT_CHUNK_SIZE);
  if( chunk_len ) {
    memcpy(gfile->pending_data, chunk_data, chunk_len);
  }
  bson_destroy( &chk );
  return MONGO_OK;
}
...
MONGO_EXPORT int gridfile_writer_done(gridfile *gfile) {
  ...
  response = gridfs_insert_file(gfile->gfs, gfile->remote_name, gfile->id, gfile->length, gfile->content_type);
...
int gridfs_store_file(gridfs *gfs, const char *filename, const char *remotename, const char *contenttype) {
  ...
  do {
    oChunk = chunk_new(id, chunkNumber, buffer, (int)chunkLen);
    mongo_insert(gfs->client, gfs->chunks_ns, oChunk, NULL);
    chunk_free(oChunk);
    length += chunkLen;
    chunkNumber++;
...
MONGO_EXPORT gridfs_offset gridfile_read(gridfile *gfile, gridfs_offset size, char *buf) {
  ...
  for (i = 0; i < total_chunks; i++) {
    ...
    if (bytes_left > chunk_len) {
      memcpy(buf, chunk_data, (size_t)chunk_len);
...
void gridfile_remove_chunks( gridfile *gfile, int deleteFromChunk){
  bson q;
